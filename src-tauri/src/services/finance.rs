...
pub fn calculate_balance(transactions: &[Transaction]) -> Decimal {
    transactions.iter().fold(Decimal::ZERO, |balance, transaction| match transaction.kind {
...
pub fn monthly_balance(transactions: &[Transaction], year: i32, month: u32) -> Decimal {
    let filtered = transactions.iter().filter(|transaction| {
        transaction.date.year() == year && transaction.date.month() == month
    });

    filtered.fold(Decimal::ZERO, |balance, transaction| match transaction.kind {
        TransactionType::Income => balance + transaction.amount,
        TransactionType::Expense => balance - transaction.amount,
    })
}
...
pub fn expenses_by_category(transactions: &[Transaction]) -> BTreeMap<Uuid, Decimal> {
    ...
    for transaction in transactions.iter().filter(|item| item.kind == TransactionType::Expense) {
...
pub fn expense_percentages(totals: &BTreeMap<Uuid, Decimal>) -> BTreeMap<Uuid, Decimal> {
    ...
    if total <= Decimal::ZERO {
        return totals.keys().copied().map(|id| (id, Decimal::ZERO)).collect();
    }
    ...
    let mut result: Vec<Transaction> = transactions
        ...
        .filter(|transaction| {
            filter.start.map_or(true, |start| transaction.date >= start)
                ...
                && filter.category_id.map_or(true, |category| transaction.category_id == Some(category))
                && filter.debit_card_id.map_or(true, |card| transaction.debit_card_id == Some(card))
        })
        .cloned()
        .collect();

    result.sort_by(|a, b| b.date.cmp(&a.date).then_with(|| b.created_at.cmp(&a.created_at)));
    result
}
...
mod tests {
    ...
    fn transaction(kind: TransactionType, amount: Decimal, category_id: Option<Uuid>, day: u32) -> Transaction {
        Transaction {
            id: Uuid::new_v4(),
            kind,
            amount,
