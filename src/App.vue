...
import { LayoutDashboard, Moon, Plus, Sun, WalletCards, Zap } from 'lucide-vue-next'
...
const activeView = ref<'dashboard' | 'wallet'>('dashboard')
...
const pageTitle = computed(() => activeView.value === 'dashboard' ? 'Meu dinheiro' : 'Carteira')
...
async function saveQuickExpense(input: NewTransactionInput) { await store.createTransaction(input); showQuickExpense.value = false; quickCardId.value = undefined }
...
onBeforeUnmount(() => removeDeepLinkListener?.())
</script>
...
<template>
  <div class="min-h-screen bg-slate-50 pb-28 transition-colors dark:bg-slate-950 sm:pb-0">
    <header class="sticky top-0 z-30 border-b border-slate-200/70 bg-white/90 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/90">
      <div class="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:px-6 sm:py-4">
        <div class="flex min-w-0 items-center gap-3">
          ...
          <div class="min-w-0"><div class="flex items-center gap-2"><p class="text-sm font-black tracking-tight">Pingo</p><span class="rounded-full bg-emerald-100 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">beta</span></div><h1 class="truncate text-xs font-semibold text-slate-500 sm:text-sm">{{ pageTitle }}</h1></div>
        </div>
    ...
    <WalletView v-else :focus-card-id="walletFocusCardId" @quick-expense="openQuickExpense" />

    <nav class="fixed inset-x-3 bottom-[calc(.75rem+env(safe-area-inset-bottom))] z-40 grid grid-cols-[1fr_82px_1fr] items-end rounded-[1.6rem] border border-slate-200/80 bg-white/95 p-1.5 shadow-2xl backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/95 sm:hidden">
      ...
      <button class="relative -top-5 mx-auto grid size-16 place-items-center rounded-[1.4rem] bg-emerald-400 text-slate-950 shadow-xl shadow-emerald-500/30 active:scale-95" aria-label="Registrar gasto rápido" @click="openQuickExpense()"><Plus :size="30" stroke-width="3" /><span class="absolute -bottom-5 text-[10px] font-black text-slate-700 dark:text-slate-300">Gasto</span></button>
