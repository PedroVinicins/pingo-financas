...
import { Bell, ChartPie, LayoutDashboard, Moon, PiggyBank, Sparkles, Sun, WalletCards, X, Zap } from 'lucide-vue-next'
...
const activeView = ref<'dashboard' | 'expenses' | 'wallet' | 'vaults'>('dashboard')
...
const pageTitle = computed(() => ({ dashboard: 'Meu dinheiro', expenses: 'Gastos e economias', wallet: 'Carteira', vaults: 'Cofres' })[activeView.value])
...
<template>
  <div class="min-h-screen bg-slate-50 pb-28 transition-colors dark:bg-slate-950 sm:pb-0">
    <header class="sticky top-0 z-30 border-b border-slate-200/70 bg-white/90 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/90">
      <div class="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:px-6 sm:py-4">
        <div class="flex min-w-0 items-center gap-3">
          ...
          <div class="min-w-0"><div class="flex items-center gap-2"><p class="text-sm font-black tracking-tight">Pingo</p><span class="rounded-full bg-emerald-100 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">v0.6.0</span></div><h1 class="truncate text-xs font-semibold text-slate-500 sm:text-sm">{{ pageTitle }}</h1></div>
        </div>
        ...
        <nav class="hidden items-center rounded-2xl bg-slate-100 p-1 dark:bg-slate-900 sm:flex">
          ...
          <button class="inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold" :class="activeView === 'expenses' ? 'bg-white shadow-sm dark:bg-slate-800' : 'text-slate-500'" @click="activeView = 'expenses'"><ChartPie :size="17" /> Gastos</button>
    ...
    <nav class="fixed inset-x-3 bottom-[calc(.75rem+env(safe-area-inset-bottom))] z-40 grid grid-cols-[1fr_1fr_84px_1fr_1fr] items-end rounded-[1.6rem] border border-slate-200/80 bg-white/95 p-1.5 shadow-2xl backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/95 sm:hidden">
      ...
      <button class="grid place-items-center gap-1 rounded-2xl py-2 text-[11px] font-black" :class="activeView === 'expenses' ? 'text-slate-950 dark:text-white' : 'text-slate-400'" @click="activeView = 'expenses'"><ChartPie :size="20" /> Gastos</button>
