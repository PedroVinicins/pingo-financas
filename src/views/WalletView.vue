...
<template>
  <main class="mx-auto max-w-[1260px] px-5 pb-8 pt-5 sm:px-8 sm:py-8 lg:px-12">
    <section class="flex items-end justify-between gap-4">
      <div class="min-w-0"><p class="text-sm font-semibold text-brand">Pingo Wallet</p><h2 class="text-[30px] font-extrabold tracking-[-.04em] sm:text-[38px]">Sua carteira</h2><p class="mt-1 text-sm text-subtle">Cartões de pagamento, ingressos, documentos e QR Codes no mesmo lugar.</p></div>
      <button class="pingo-interactive hidden min-h-12 items-center gap-2 rounded-full bg-brand px-5 font-bold text-white shadow-lg shadow-violet-500/20 sm:flex" @click="showAddMenu = true"><Plus :size="18" /> Novo cartão</button>
    </section>
    ...
    <section v-if="walletEntriesCount" class="mt-5">
      <div class="no-scrollbar -mx-5 flex snap-x snap-mandatory gap-3 overflow-x-auto px-5 pb-5 sm:mx-0 sm:px-0" data-no-page-swipe>
        <button v-for="card in store.debitCards" :key="card.id" class="w-[86vw] max-w-[350px] shrink-0 snap-center text-left transition duration-300 ease-pingo" :class="selectedCardId === card.id ? 'translate-y-0 opacity-100' : 'translate-y-2 scale-[.96] opacity-55'" @click="selectCard(card.id)"><DebitCardVisual :card="card" /></button>
        ...
        <button class="grid aspect-[1.586/1] w-[70vw] max-w-[280px] shrink-0 snap-center place-items-center rounded-[30px] border-2 border-dashed border-line bg-surface text-subtle transition hover:border-brand hover:bg-brand-soft hover:text-brand" @click="showAddMenu = true"><span class="grid place-items-center gap-2 text-sm font-bold"><Plus :size="25" /> Adicionar à carteira</span></button>
      </div>

      <template v-if="selectedCard">
        <section class="soft-shadow rounded-[var(--radius)] border border-line bg-surface p-4 sm:p-6">
          <div class="flex items-center justify-between gap-3">
            <div class="min-w-0"><div class="flex items-center gap-2"><h3 class="truncate text-xl font-extrabold">{{ selectedCard.name }}</h3><Star v-if="selectedCard.isDefault" :size="15" class="text-brand" fill="currentColor" /></div><p class="text-xs text-subtle">{{ selectedCard.issuer }} · •••• {{ selectedCard.lastFour }}</p></div>
          ...
          <div class="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <div class="min-w-0 overflow-hidden rounded-2xl bg-muted p-3"><p class="truncate text-[11px] font-bold text-subtle">Este mês</p><p class="mt-1 truncate font-extrabold tabular-nums" :title="money(monthSpentCents)">{{ money(monthSpentCents) }}</p></div>
            <div class="min-w-0 overflow-hidden rounded-2xl bg-muted p-3"><p class="truncate text-[11px] font-bold text-subtle">Média/compra</p><p class="mt-1 truncate font-extrabold tabular-nums" :title="money(averagePurchaseCents)">{{ money(averagePurchaseCents) }}</p></div>
            <div class="min-w-0 overflow-hidden rounded-2xl bg-muted p-3"><p class="truncate text-[11px] font-bold text-subtle">Compras</p><p class="mt-1 truncate font-extrabold tabular-nums" :title="String(selectedTransactions.length)">{{ selectedTransactions.length }}</p></div>
            <div class="min-w-0 overflow-hidden rounded-2xl bg-muted p-3"><p class="truncate text-[11px] font-bold text-subtle">Histórico</p><p class="mt-1 truncate font-extrabold tabular-nums" :title="money(totalSpentCents)">{{ money(totalSpentCents) }}</p></div>
          </div>

          <div v-if="monthLimitCents" class="mt-4"><div class="mb-1.5 flex justify-between text-xs font-bold"><span>Controle mensal</span><span class="text-subtle">{{ limitProgress.toFixed(0) }}%</span></div><div class="h-2 overflow-hidden rounded-full bg-muted"><div class="h-full rounded-full bg-brand" :style="{ width: `${limitProgress}%` }"></div></div></div>

          <div class="mt-5 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            <button :disabled="selectedCard.isFrozen" class="pingo-interactive col-span-2 flex items-center justify-center gap-2 rounded-full bg-brand px-5 py-3.5 font-extrabold text-white shadow-lg shadow-violet-500/20 disabled:cursor-not-allowed disabled:opacity-40 sm:col-auto" @click="emit('quickExpense', selectedCard.id)"><Zap :size="18" fill="currentColor" /> Gasto neste cartão</button>
            <button class="surface-control flex items-center justify-center gap-2 rounded-2xl px-3 py-3 text-sm font-bold" @click="showStyleEditor = true"><Palette :size="17" /> Personalizar</button>
            <button :disabled="shortcutBusy" class="surface-control flex items-center justify-center gap-2 rounded-2xl px-3 py-3 text-sm font-bold disabled:opacity-50" @click="createHomeShortcut"><Smartphone :size="17" /> {{ shortcutBusy ? 'Criando…' : copied ? 'Solicitado' : 'Tela inicial' }}</button>
            <button class="surface-control flex items-center justify-center gap-2 rounded-2xl px-3 py-3 text-sm font-bold" @click="toggleFrozen"><Lock :size="17" /> {{ selectedCard.isFrozen ? 'Descongelar' : 'Congelar' }}</button>
            <button v-if="!selectedCard.isDefault" class="surface-control flex items-center justify-center gap-2 rounded-2xl px-3 py-3 text-sm font-bold" @click="makeDefault"><Check :size="17" /> Principal</button>
          ...
          <p class="mt-3 text-xs text-subtle">“Tela inicial” pede ao Android para criar um ícone deste cartão. Ele abre a Carteira diretamente no cartão escolhido.</p>
        </section>

        <div class="mt-4 grid gap-4 lg:grid-cols-[1fr_.38fr]">
          <section class="soft-shadow min-w-0 overflow-hidden rounded-[var(--radius)] border border-line bg-surface p-4 sm:p-6"><div class="mb-3 flex min-w-0 items-center gap-2"><ReceiptText :size="19" class="shrink-0 text-brand" /><div class="min-w-0 overflow-hidden"><p class="truncate text-xs font-bold text-subtle">Histórico editável</p><h3 class="truncate font-extrabold" :title="`Compras com ${selectedCard.name}`">Compras com {{ selectedCard.name }}</h3></div></div><TransactionList :transactions="selectedTransactions" :categories="store.categories" :cards="store.debitCards" editable @edit="editingTransaction = $event" /></section>
          <aside class="hero-shadow rounded-[30px] bg-hero p-5 text-white"><Building2 :size="20" /><p class="mt-5 text-xs font-bold text-white/45">Saldo da conta</p><p class="mt-1 text-3xl font-extrabold">{{ money(store.balanceCents) }}</p><p class="mt-3 text-xs leading-5 text-white/45">O Pingo não cria um saldo artificial para cada cartão. Todo gasto reduz o mesmo caixa.</p></aside>
        </div>
      </template>
      <section v-else-if="selectedWalletItem" class="soft-shadow overflow-hidden rounded-[var(--radius)] border border-line bg-surface"><button v-if="selectedWalletItem.fileDataUrl && selectedWalletItem.mimeType?.startsWith('image/')" class="block max-h-[420px] w-full overflow-hidden bg-muted" :aria-label="`Ampliar ${selectedWalletItem.title}`" @click="previewingWalletItem = selectedWalletItem"><img :src="selectedWalletItem.fileDataUrl" :alt="selectedWalletItem.title" class="mx-auto max-h-[420px] w-full object-contain" /></button><div v-else class="grid min-h-56 place-items-center bg-brand-soft text-brand"><QrCode v-if="selectedWalletItem.kind === 'qr_code'" :size="72" /><FileText v-else :size="72" /></div><div class="p-5 sm:p-6"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><span class="text-xs font-extrabold uppercase tracking-wider text-brand">Cartão ao vivo · {{ walletKindLabel(selectedWalletItem.kind) }}</span><h3 class="mt-1 truncate text-2xl font-extrabold" :title="selectedWalletItem.title">{{ selectedWalletItem.title }}</h3><p v-if="selectedWalletItem.issuer" class="mt-1 truncate text-sm text-subtle" :title="selectedWalletItem.issuer">{{ selectedWalletItem.issuer }}</p></div><button class="grid size-11 shrink-0 place-items-center rounded-xl border border-rose-200 text-rose-600 dark:border-rose-900" :aria-label="`Remover ${selectedWalletItem.title}`" @click="removingWalletItem = selectedWalletItem"><Trash2 :size="18" /></button></div><p v-if="selectedWalletItem.expiresAt" class="mt-4 flex items-center gap-2 text-sm font-bold text-amber-600"><CalendarDays :size="17" /> Válido até {{ displayDate(selectedWalletItem.expiresAt) }}</p><p v-if="selectedWalletItem.qrValue" class="mt-4 break-all rounded-xl bg-muted p-3 font-mono text-xs">{{ selectedWalletItem.qrValue }}</p><p v-if="selectedWalletItem.notes" class="mt-4 text-sm leading-relaxed text-subtle">{{ selectedWalletItem.notes }}</p><button v-if="selectedWalletItem.fileDataUrl && selectedWalletItem.mimeType?.startsWith('image/')" class="mt-5 w-full rounded-full bg-brand px-4 py-3 text-center text-sm font-extrabold text-white" @click="previewingWalletItem = selectedWalletItem">Ver imagem em tela cheia</button><a v-else-if="selectedWalletItem.fileDataUrl" :href="selectedWalletItem.fileDataUrl" :download="selectedWalletItem.fileName || selectedWalletItem.title" class="mt-5 block rounded-full bg-brand px-4 py-3 text-center text-sm font-extrabold text-white">Abrir arquivo</a><p class="mt-4 text-xs text-subtle">Este cartão fica somente neste dispositivo e está disponível offline.</p></div></section>
    </section>

    <section v-else class="soft-shadow mt-6 grid min-h-[360px] place-items-center rounded-[30px] border border-dashed border-line bg-surface p-8 text-center"><div class="max-w-sm"><div class="mx-auto grid size-16 place-items-center rounded-3xl bg-brand-soft text-brand"><WalletCards :size="30" /></div><h3 class="mt-5 text-2xl font-extrabold">Monte sua Pingo Wallet</h3><p class="mt-2 text-sm text-subtle">Adicione um cartão de débito, ingresso, documento ou QR Code.</p><button class="pingo-interactive mt-5 rounded-full bg-brand px-5 py-3 font-extrabold text-white" @click="showAddMenu = true">Adicionar primeiro cartão</button></div></section>
  </main>
