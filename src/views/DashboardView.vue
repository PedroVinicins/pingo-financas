...
function pointerEnd() {
  if (draggingWidget.value && touchDropTarget.value) moveWidget(draggingWidget.value, touchDropTarget.value)
  draggingWidget.value = null
...
<template>
  <main class="mx-auto max-w-6xl px-4 py-5 sm:px-6 sm:py-8">
    ...
    <section v-if="visibleWidgets.length" class="grid grid-cols-1 gap-4 lg:grid-cols-12">
      <div v-for="widget in visibleWidgets" :key="widget.id" :data-dashboard-widget="widget.id" class="relative" :class="[widgetClass(widget.size), draggingWidget === widget.id ? 'rounded-[2rem] opacity-60 ring-2 ring-emerald-400' : '']" @dragover.prevent @drop="dropWidget(widget.id, $event)">
