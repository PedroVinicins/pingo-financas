...
async function confirmRemoval() {
  ...
  removing.value = true
  try {
    await store.removeVault(confirmingRemoval.value.id)
    store.showFeedback('Cofre removido. O saldo voltou a ficar disponível.', 'success')
    confirmingRemoval.value = null
  } catch (cause) { store.reportError(cause, 'Não foi possível remover o cofre.') }
...
<template>
  <main class="mx-auto max-w-[1440px] px-5 py-5 sm:px-7 sm:py-8 lg:px-10">
    ...
    <section class="relative mt-5 overflow-hidden rounded-[2rem] bg-hero p-6 text-white shadow-float sm:p-8"><div class="absolute right-0 top-0 size-44 rounded-full bg-brand/15 blur-3xl"></div><div class="relative flex items-start justify-between gap-4"><div><div class="grid size-12 place-items-center rounded-2xl bg-white/10 text-violet-300"><PiggyBank :size="25" /></div><p class="mt-5 text-sm font-semibold text-white/55">Total nos porquinhos</p><h3 class="mt-1 truncate text-4xl font-extrabold tracking-tight sm:text-5xl" :title="money(store.vaultTotalCents)">{{ money(store.vaultTotalCents) }}</h3><p class="mt-3 truncate text-sm font-semibold text-white/55" :title="money(store.availableBalanceCents)">Na conta principal: {{ money(store.availableBalanceCents) }}</p></div><Sparkles :size="34" class="relative text-violet-300" /></div></section>
