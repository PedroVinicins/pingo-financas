...
async function downloadBackup() {
  ...
  try {
    await exportBackup({
      ...
      monthlyReserveRules: [...store.monthlyReserveRules],
      digitalWalletItems: [...store.digitalWalletItems],
  ...
  <div class="fixed inset-0 z-[80] flex items-end bg-slate-950/50 backdrop-blur-[2px] sm:items-center sm:justify-center sm:p-4" @click.self="emit('close')">
    <section class="max-h-[92dvh] w-full overflow-y-auto rounded-t-[2rem] bg-white p-5 shadow-2xl dark:bg-slate-900 sm:max-w-lg sm:rounded-[2rem] sm:p-6" role="dialog" aria-modal="true" aria-labelledby="app-settings-title">
      ...
      <div class="mt-5 grid gap-3">
        ...
        <article class="flex items-start gap-3 rounded-2xl border border-slate-200 p-4 dark:border-slate-800"><Smartphone :size="20" class="mt-0.5 shrink-0 text-amber-600" /><div><p class="text-sm font-black">Pingo 0.9.0</p><p class="mt-1 text-xs leading-relaxed text-slate-500">Extratos, saldos e porquinhos finalmente falando a mesma língua.</p></div></article>
      </div>
