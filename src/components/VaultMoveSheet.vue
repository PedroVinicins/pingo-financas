...
const props = defineProps<{ vault: Vault; initialKind?: VaultMovementType }>()
...
function submit() {
  ...
  if (!/^\d+(\.\d{1,2})?$/.test(normalized) || Number(normalized) <= 0) { error.value = 'Informe um valor maior que zero.'; return }
...
<template>
  <div class="fixed inset-0 z-[75] flex items-end bg-slate-950/55 sm:items-center sm:justify-center sm:p-4" @click.self="emit('close')">
    <form class="w-full rounded-t-[2rem] bg-white p-5 dark:bg-slate-900 sm:max-w-md sm:rounded-[2rem] sm:p-6" @submit.prevent="submit">
      ...
      <label class="mt-5 grid gap-1.5 text-sm font-semibold">Valor<input v-model="amount" autofocus inputmode="decimal" placeholder="0,00" class="rounded-2xl border border-slate-200 bg-transparent px-4 py-4 text-3xl font-black dark:border-slate-700" /></label>
      <p class="mt-3 text-xs text-slate-500">Registre aqui depois de fazer a transferência no seu banco.</p>
