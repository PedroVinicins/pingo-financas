...
<template>
  <AppModal as="form" aria-labelledby="digital-wallet-title" root-class="z-[90]" panel-class="p-5 sm:max-w-lg sm:p-6" :closeable="!busy" @close="emit('close')" @submit="submit">
      <div class="flex items-start justify-between gap-3"><div><p class="text-sm font-bold text-violet-600">Carteira ao vivo</p><h2 id="digital-wallet-title" class="break-words text-xl font-black">Criar cartão com sua imagem</h2><p class="mt-1 text-xs text-slate-500">Escolha uma foto da galeria para usar como capa.</p></div><button type="button" :disabled="busy" class="pingo-modal-close" aria-label="Fechar" @click="emit('close')"><X :size="19" /></button></div>
      ...
      <button type="submit" :disabled="reading || busy" class="pingo-modal-footer btn mt-5 min-h-12 w-full rounded-2xl border-0 bg-violet-500 font-black text-white disabled:cursor-wait disabled:opacity-50">{{ busy ? 'Guardando…' : 'Guardar na carteira' }}</button>
  </AppModal>
</template>
