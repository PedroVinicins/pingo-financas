...
import { FileUp, ShieldCheck, X } from 'lucide-vue-next'
...
<template>
  <div class="fixed inset-0 z-[90] flex items-end bg-slate-950/55 sm:items-center sm:justify-center sm:p-4" @click.self="emit('close')">
    <form class="max-h-[94dvh] w-full overflow-y-auto rounded-t-[2rem] bg-white p-5 shadow-2xl dark:bg-slate-900 sm:max-w-lg sm:rounded-[2rem] sm:p-6" @submit.prevent="submit">
      <div class="flex items-start justify-between gap-3"><div><p class="text-sm font-bold text-violet-600">Carteira ao vivo</p><h2 class="text-xl font-black">Guardar ingresso ou documento</h2></div><button type="button" class="grid size-10 place-items-center rounded-xl bg-slate-100 dark:bg-slate-800" aria-label="Fechar" @click="emit('close')"><X :size="19" /></button></div>
      ...
      <label class="mt-4 flex cursor-pointer items-center gap-3 rounded-2xl border-2 border-dashed border-violet-200 p-4 dark:border-violet-900"><FileUp :size="22" class="text-violet-600" /><span class="min-w-0"><strong class="block text-sm">{{ reading ? 'Lendo arquivo…' : form.fileName || 'Escolher imagem ou PDF' }}</strong><span class="block text-xs text-slate-500">JPG, PNG, WebP ou PDF · até 3 MB</span></span><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" class="sr-only" @change="pickFile" /></label>
      ...
      <button :disabled="reading" class="mt-5 w-full rounded-2xl bg-violet-500 py-3.5 font-black text-white disabled:opacity-50">Guardar na carteira</button>
    </form>
  </div>
</template>
