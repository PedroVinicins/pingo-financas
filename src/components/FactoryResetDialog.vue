  ...
  <Teleport to="body">
    <div class="fixed inset-0 z-[120] grid place-items-end bg-slate-950/65 backdrop-blur-[2px] sm:place-items-center sm:p-4" @click.self="!busy && emit('cancel')">
      <section class="w-full rounded-t-[2rem] bg-white p-5 shadow-2xl dark:bg-slate-900 sm:max-w-md sm:rounded-[2rem] sm:p-6" role="alertdialog" aria-modal="true" aria-labelledby="factory-reset-title">
        ...
        <div class="mt-5 grid grid-cols-2 gap-2"><button :disabled="busy" class="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-black dark:border-slate-700" @click="emit('cancel')">Cancelar</button><button :disabled="busy || !allowed" class="rounded-2xl bg-rose-600 px-4 py-3 text-sm font-black text-white disabled:opacity-40" @click="emit('confirm')">{{ busy ? 'Apagando…' : 'Apagar tudo' }}</button></div>
      </section>
    </div>
  </Teleport>
</template>
